...
3. **検索は合計12回前後に収め、多くとも15回を超えないこと。** 内訳の目安:
   ...
   - **全体の処理時間は10分以内を目標とし、30分を超えないこと。** 目標時間を超えそうな場合は、残りのトピックの検索を打ち切り、それまでに集まった記事だけで手順6以降を進める
...
5. 各記事について「インパクトの大きさ」（株価・業績への影響、大型提携・M&Aなど）と「情報量の多さ」（記事の深さ・具体性）の2軸で、一読しての直感的な判断でざっくり評価する（1本ごとに深く考え込まない）。**両方が高い記事だけ** を `tier: "large"` の候補にする。インパクトが大きくても情報量が少ない記事は `tier: "simple"` に回す。
...
- **速度優先。** 検索回数と処理時間の上限（手順3参照）を必ず守ること。網羅性より「時間内に完走してdata/にコミットされること」を優先する。時間切れが近い場合は、要約を短くしてでも手順9のpushまで終わらせる
